...
# Import faster-whisper for speech-to-text
...
class Gemma3Processor:
    """Handles text generation using Gemma 3 model"""
    _instance = None
    ...
    def __init__(self):
        ...
        logger.info(f"Using device for Gemma-3: {self.device}")
        ...
        logger.info("Gemma-3 model ready for text generation")
    ...
    async def process_audio(self, audio_bytes):
        ...
        try:
            ...
            segments, info = await asyncio.get_event_loop().run_in_executor(
                None, 
                lambda: self.whisper_model.transcribe(audio_array, language="en")
            )
            ...
            if not transcription or len(transcription) < 3:
                ...
                return None, "NOISE:NO_SPEECH", False
                
            return await self.generate_streaming(transcription)
            
        except Exception as e:
            logger.error(f"Error processing audio: {e}")
            return None, f"Error processing audio: {str(e)}", False
    
    async def generate_streaming(self, text_query, initial_chunks=3):
        """Generate a response using text input with streaming"""
        async with self.lock:
            try:
                # Create chat messages for Gemma 3
                messages = [
                    {
                        "role": "system",
                        "content": [{"type": "text", "text": "You are a helpful voice assistant. Respond conversationally and keep responses clear and concise for voice output."}]
                    },
                    {
                        "role": "user", 
                ...
                logger.info(f"Gemma-3 initial generation: '{initial_text}' ({len(initial_text)} chars)")
                
                return streamer, initial_text, initial_collection_stopped_early
                
            except Exception as e:
                logger.error(f"Gemma-3 streaming generation error: {e}")
...
async def handle_client(websocket):
    ...
    try:
        ...
        gemma3_processor = Gemma3Processor.get_instance()
            ...
            while True:
                try:
                    ...
                    if speech_segment:
                        ...
                        try:
                            # Create generation task - pass audio to Gemma-3 pipeline
                            logger.info("Creating new Gemma-3 generation task for speech segment")
                            generation_task = asyncio.create_task(
                                gemma3_processor.process_audio(speech_segment)
                            )
                            ...
                            try:
                                logger.info("Awaiting initial text generation from Gemma-3")
                                        ...
                                        if initial_collection_stopped_early:
                                            ...
                                            try:
                                                ...
                                                if collected_chunks:
                                                    ...
                                                    gemma3_processor.update_history_with_complete_response(
